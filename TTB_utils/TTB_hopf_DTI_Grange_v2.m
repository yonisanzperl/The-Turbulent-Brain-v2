...
% EDR +LR
if strcmp(Cfg.connectivity, 'EDR+LR')
    C(IClong)=Clong(IClong);
    factor=max(max(C));
    C=C/factor*0.2;
end
% EDR
if strcmp(Cfg.connectivity, 'EDR')    
    factor=max(max(C));
    C=C/factor*0.2;
end
% SC
if strcmp(Cfg.connectivity, 'SC')
    factor=max(max(C));
    C=C/factor*0.2;
end
...
for nG=1:size(G_range,2)
   ...
   [model_output(nG)] = TTB_turbulence_node_empirical_metaesta_RSN(Cfg, CoG,ts_out,RSN);               
