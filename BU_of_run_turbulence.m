...
%          end
       ...
       % [Clong lambda_emp numexcSC valexcSC]=TTB_SC_analysis_longrange_EDR_DTI(Cfg,CoG,SC);
        ...
        cd(metadata.outdir)
        save('Results_modelfree.mat','output','CoG','RSN','metadata','Cfg');
    ...
    case 'Model-based'
        ...
        a2 = questdlg('Do you have fMRI data?', ...
            'The Turbulent Brain', ...
            'Yes','No','No thank you');
        ...
        switch a2
            ...
            case 'No'
                ...
                [Answer,Answer0,Cancelled]=IN_turbulence_mb3;
                metadata.saveFile = Answer.saveFile;
                metadata.currentDir=pwd;
                ...
                [Clong lambda_emp numexcSC valexcSC]=TTB_SC_analysis_longrange_EDR_DTI(Cfg,CoG,SC);
                f_diff=ones(1,Cfg.nNodes)*0.025;
                if Cfg.PlotYes
                    figure('Name','Model turbulence');
                end
                for kk=1:Cfg.nBrainStates
                    fprintf('Modeled Brain state: %d/%d\n',kk,Cfg.nBrainStates)
                    [model_output(kk,:)]=TTB_hopf_DTI_Grange_v2(Cfg,CoG,SC,f_diff,RSN,Clong, lambda_emp, numexcSC, valexcSC);
                    if Cfg.PlotYes
                        subplot(1,Cfg.nBrainStates,kk)
                        for ll=1:size(G_range,2)
                            turbu(:,ll)=mean(model_output(kk,ll).Turbulence_sub,2);
                        end
                        count=1;
                        for rr=LAMBDA
                            hold on
                            plot(G_range,squeeze(turbu(count,:)));legend(num2str(rr))
                            count=count+1;
                        end
                    end
                    [aux indsca]=min(abs(LAMBDA-Cfg.lamICS));
                    LAMBDA = LAMBDA(indsca);
                    [max_val(kk) indx_max(kk)]=max((squeeze(turbu(indsca,:))));  
                    if Cfg.PertuYes
                        fprintf('Perturbed Brain state: %d/%d\n',kk,Cfg.nBrainStates)
                        [IC(kk,:) S(kk,:) ]= TTB_infocapacity_hopf(Cfg.nTrials,kk,G_range(indx_max(kk)),Cfg,CoG,SC,f_diff,Clong, lambda_emp, numexcSC, valexcSC);
                    end
                end
                ...
                if Cfg.PertuYes
                    TTB_plot_modelIC(model_output,Cfg, IC, S, metadata)
                    cd(metadata.outdir)
                    save('TTBoutput_modelbased.mat','model_output','IC','S','metadata','Cfg','G_range');
